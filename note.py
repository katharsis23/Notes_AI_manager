...
async def main():
    ...
    if not raw_topic:
        console.print("[bold red]Error: Empty prompt.[/bold red]")
        return
    ...
    with console.status(
        f"[bold green]Generating note for '{raw_topic}'...[/bold green]"
    ):
        note_path = await pipeline.generate_and_save(raw_topic)

    if note_path:
        console.print(f"\n[bold green]✔ Note saved: {note_path}[/bold green]")
    else:
        console.print("\n[bold red]✘ Failed to save the note.[/bold red]")
